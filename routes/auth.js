...
router.post("/register", async (req, res) => {
    ...
    try {
        if (userUsername) {
            res.status(401).json({ message: "Böyle bir kullanıcı ismi bulunmaktadır." });
        } else if (userMail) {
            res.status(401).json({ message: "Böyle bir E-mail adresi bulunmaktadır." });
        } else {
...
router.post("/login", async (req, res) => {
    try {
        ...
        if (!user) {
            res.status(401).json("Yanlış kullanıcı adı!")
        } else if (!validPassword) {
            res.status(400).json("Yanlış şifre!")
        } else {
